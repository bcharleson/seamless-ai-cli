...
export function output(data: unknown, options: GlobalOptions = {}): void {
  ...
  if (options.fields && typeof data === 'object' && data !== null) {
    ...
    if (Array.isArray(data)) {
      ...
      // Unwrap Seamless.ai paginated search responses: {data: [...]} → apply fields to each item
  ...
  if (options.output === 'pretty') {
    ...
    // Default: compact JSON for machine/agent consumption
...
function pickFields(
  ...
  for (const field of fields) {
    // Support nested paths like "contactLocation.city" or "jobHistory.0.company"
    if (field.includes('.')) {
